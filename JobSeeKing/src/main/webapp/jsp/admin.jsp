			...
			<div class="search-wrapper" style="margin-top: -5rem;">
				...
				<ul class="nav nav-tabs search-nav-tabs" id="myTab" role="tablist">
					<li class="nav-item search-nav-item"><a class="nav-link"
						href="<c:url value="/?loggedin=admin&name=${name}"/>">TRANG
							CHỦ</a></li>
					<li class="nav-item search-nav-item"><a class="nav-link"
						href="<c:url value="/job?user=ADMIN&username=${name }"/>">VIỆC
							LÀM</a></li>
					<li class="nav-item search-nav-item"><a class="nav-link"
						href="#">DASHBOARD</a></li>
				</ul>
										...
										<div class="job pagi">
											<div class="job-content">
												...
												<div class="job-desc">
													<div class="job-title">
														<a
															href="<c:url value="/detail?jobID=${job.job_id }&user=ADMIN"/>">${job.title }</a>
													</div>
												...
												<div class="wrap-btn-appl">
													<form action="confirm?jobID=${job.job_id }" method="post">
														<input type="hidden" name="${_csrf.parameterName}"
															value="${_csrf.token}" /> <input class="btn btn-success"
															type="submit" name="" value="Duyệt" /> <input
															class="btn btn-warning" name="" value="${job.fee}"
															readonly />

													</form>
													<script>
														window.onload = function() {
															// Lấy tất cả các phần tử input có class là 'btn btn-warning'
															let buttons = document
																	.querySelectorAll('input.btn.btn-warning');
															// Duyệt qua từng phần tử
															buttons
																	.forEach(function(
																			button) {
																		// Kiểm tra giá trị của từng nút
																		if (button.value !== 'Premium') {
																			// Ẩn nút nếu giá trị khác 'Premium'
